...
extension AttributeType {
    ...
    nonisolated(unsafe) static let regexes: [NSRegularExpression : ([Int]) -> AttributeType] = [
        ...
        compile(regex: "^Intensity(?<n>[0-9]+)MSpeed$") : { e in .intensityMovementSpeed(n: e[0]) },
        compile(regex: "^Position(?<n>[0-9]+)MSpeed$") : { e in .positionMovementSpeed(n: e[0]) },
        compile(regex: "^ColorMix(?<n>[0-9]+)MSpeed$") : { e in .colorMixMovementSpeed(n: e[0]) },
        compile(regex: "^ColorWheelSelect(?<n>[0-9]+)Speed$") : { e in .colorWheelSelectMovementSpeed(n: e[0]) },
        compile(regex: "^GoboWheel(?<n>[0-9]+)MSpeed$") : { e in .goboWheelMovementSpeed(n: e[0]) },
        compile(regex: "^Iris(?<n>[0-9]+)MSpeed$") : { e in .irisMovementSpeed(n: e[0]) },
        compile(regex: "^Prism(?<n>[0-9]+)MSpeed$") : { e in .prismMovementSpeed(n: e[0]) },
        compile(regex: "^Focus(?<n>[0-9]+)MSpeed$") : { e in .focusMovementSpeed(n: e[0]) },
        compile(regex: "^Frost(?<n>[0-9]+)MSpeed$") : { e in .frostMovementSpeed(n: e[0]) },
        compile(regex: "^Zoom(?<n>[0-9]+)MSpeed$") : { e in .zoomMovementSpeed(n: e[0]) },
        compile(regex: "^Frame(?<n>[0-9]+)MSpeed$") : { e in .frameMovementSpeed(n: e[0]) },
        compile(regex: "^Global(?<n>[0-9]+)MSpeed$") : { e in .globalMovementSpeed(n: e[0]) },
